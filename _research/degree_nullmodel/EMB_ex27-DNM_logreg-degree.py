...
def experiment(system, layer_pair, parameters, hyperparameters, experiment_setup):
    ...
    input_data_ = [
        dataio.get_input_filehandle(system, ROOT="./", DIR="data/", PREFACE=f"shuffled_{x}")
        for x in range(1, 11)
    ]
    ...
    progbar_shuffles = tqdm(input_data_, desc="Shuffled null models", position=3, leave=False, colour="red")
    progbar_thetas = tqdm(thetas, desc="theta", position=4, leave=False, colour="blue")
    ...
    # >>> Sweep parameters >>>
    for input_data in progbar_shuffles:
