...
def get_data(cache_fp):
    ...
    logging.info(f"Remnants have {edgelists[0].number_of_nodes()} and {edgelists[1].number_of_nodes()} nodes")
...
def evaluate(cache_fp):
    ...
    logging.info(f"Starting {cache_fp}")
...
def analysis(
    ...
    dir_ = [
        ...
        if query in fp and "arxiv" not in fp and "l1-1_l2-2" not in fp
    ]
    
    logging.info(f"Reconstructing {len(dir_)} CachedEmbeddings")
    
    # --- Computations ---
    logging.info(f"Using {processes} workers with chunks of size {chunksize}")
    #with Pool(processes=processes) as p:
        # records = p.map(evaluate, dir_, chunksize=chunksize)
    records = [evaluate(x) for x in dir_]
