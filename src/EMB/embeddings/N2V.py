...
def _check_kwargs(_panic=PANIC, **kwargs):
    ...
    for key in keys:
        if key not in ACCEPTED_PARAMS_N2V:
            if _panic:
                raise ValueError(f"'{key}' is not a N2V parameter or not designated as mutable!")
            else:
                LOGGER.warning(f"'{key}' is not a N2V parameter or not designated as mutable! Panic disabled - dropping '{key}' from kwargs.")
                del kwargs[key]
