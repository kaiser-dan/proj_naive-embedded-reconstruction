...
def main(filepath_input, filepath_output):
    ...
    for theta, remnant_multiplex in remnant_multiplexes.items():
        mplxio.to_edgelist(
            remnant_multiplex, filepath_output.format(theta=f"{theta:.2f}")
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        regex = sys.argv[1]
    ntwks = [
        fh for fh in os.listdir(os.path.join("data", "edgelists", "")) if regex in fh
    ]
    for ntwk in ntwks:
        ...
        for rep in range(1, 6):
            filepath_output = os.path.join(
                "data",
                "remnants",
                "remnants_theta-{theta}_rep-" + str(rep) + "_" + ntwk[:-5] + ".rmnt",
            )
            print("Calculating remnants...")
            output_filehandles = [
                os.path.exists(filepath_output.format(theta=f"{THETAS[x]:.2f}"))
                for x in range(1, len(THETAS))
            ]
