...
def main(filepath_input_gt, filepath_input_rmnt, filepath_input_emb, filepath_output):
    ...
    # Layer imbalance
    zeta = len([x for x, y in y_train.items() if y == 1]) / len(y_train)
    zeta = 2 * zeta - 1
    ...
    X_train = np.array(
        [zeta * np.ones(len(training_distances)), training_distances, training_degrees]
    ).transpose()
    ...
    if "LFR" in filepath_input_emb:
        ...
        record_str += f"{model.coef_[0][0]},{model.coef_[0][1]},{model.coef_[0][2]}"
        print(record_str)
